 ...
 * @version 1.0
 * 
 * Creates a stream connection using the Streaming API for twitter. 
 * Limited to 400 keywords, 5000 users, 25 geo-locations
 * Streams do not user the rate limit, but it is limited to 1 connect per authenticated ID
 * 
 */
public class PublicStream{
     ...
     * @param tweetStream twitter stream should be created from the UI when the program begins
     */
	...
	/**
	 * Sets the listener for the stream by enabling the connection and listening for updates
	 * Depending on what comes through such as a status update, then call the appropriate DB option
	 * @throws TwitterException there is a stall in the stream or bandwith issues
	 */
	...
	/**
	 * when a user is added to the track list, this adds the user's profile into the
	 * database. This uses 1 query limit.
	 * 
	...
	/**
	 * Adds a search term to be tracked. 
	 * Similar to the search API, but single keywords are recommended due to character limit.
	...
	/**
	 * The track list is not dynamic and must be restarted to update the tracked users/words.
	 * This method restarts the stream with the new list.
	 * 
