 ...
 * @version 1.0
 * 
 * This class contains methods that handle the conversion of twitter data to data entities for the database.
 * Data retrieved from tweeter must be separated into columns then built as a sql query. 
 */
public class DB_Interface {
	...
	/**
	 * Inserts a new status into the DB. This is for public timelines that are scraped from twitter.
	 * 
	 * @param conn connection to the DB
	 * @param pt instance of a public timeline
	 * @throws SQLException thrown when an error occurs in the sql statement
	 */
	public static void insertStatus(Connection conn, PublicTimeline pt)
			throws SQLException {
		//prep sql statement
		PreparedStatement prep = conn
	...
	/**
	 * Inserts a single status from a class, used with the Search API and Streaming API
	 * 
	...
	/**
	 * Inserts a Tweet (Status from a Search API), structured differently than a full status.
	 * 
	 * @deprecated using statusID convert over to a full status and use insertStatus for consistency with Timeline and Stream.
	 * @param conn
	 * @param s
	 * @throws SQLException
	 */
	public static void insertTweet(Connection conn, Status s) throws SQLException {
		PreparedStatement prep = conn
				.prepareStatement("INSERT INTO TWEETS VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		try {
			statusBuilder(s, prep);
	...
	/**
	 * Inserts hash tag entitie into the DB.
	 * 
	 * @deprecated 
	 * @param conn the connection to the DB
	 ...
	 * @throws Exception
	 */
	public static void insertHash(Connection conn, PublicTimeline pt)
			throws Exception {
		insertStatus(conn, pt);
		PreparedStatement prep = conn
				.prepareStatement("INSERT INTO HASHTAGS VALUES(?,?,?)");

	}

	/**
	 * Takes a instance of a Status and structures the parts for insertions into the DB.
	 * It does so by setting each "?" in the prepared statement to the specified constant number.
	 * 
	...
	/**
	 * Structures a tweet to the prepared statement for insertion into the DB.
	 * 
	 * @deprecated tweets are converted into statuses so use the status builder
	 * @param tw the tweet to by inserted
	 * @param prep prepared sql statement
	 * @throws SQLException
	 */
	public static void tweetBuilder(Tweet tw, PreparedStatement prep)
			throws SQLException {

		prep.setTimestamp(CREATED, dateConvertor(tw));
		prep.setLong(ID, tw.getId());
		prep.setString(TEXT, tw.getText());
		prep.setString(SOURCE, tw.getSource());

		/*
		 * Missing Tweet items prep.setBoolean(IS_TRUNCATED, tw.isTruncated());
		 * prep.setLong(REPLY_TO, tw.getInReplyToStatusId());
		 * prep.setLong(REPLY_USER, tw.getInReplyToUserId());
		 * prep.setBoolean(FAVORITED, tw.isFavorited());
		 * prep.setString(REPLY_USER_SN, tw.getInReplyToScreenName());
		 */
		prep.setBoolean(IS_TRUNCATED, false);
		prep.setLong(REPLY_TO, -1);
		prep.setLong(REPLY_USER, -1);
		prep.setBoolean(FAVORITED, false);
		prep.setString(REPLY_USER_SN, null);

		// similar fields
		if (tw.getGeoLocation() != null) {
			prep.setString(GEOLOCATION, String.valueOf(tw.getGeoLocation()));
		} else {
			prep.setString(GEOLOCATION, null);
		}
		if (tw.getPlace() != null) {
			prep.setString(PLACE, tw.getPlace().getName());
		} else {
			prep.setString(PLACE, null);
		}

		/*
		 * Missing tweet items prep.setLong(RETWEET_COUNT,
		 * tw.getRetweetCount()); prep.setBoolean(RETWEETED_BY_ME,
		 * tw.isRetweetedByMe()); prep.setString(CONTRIBUTORS,
		 * contributorConverter(tw.getContributors()));
		 */
		prep.setLong(RETWEET_COUNT, -1);
		prep.setBoolean(RETWEETED_BY_ME, false);
		prep.setString(CONTRIBUTORS, null);

		// retrieval methods are deprecated
		prep.setString(ANNOTATIONS, "NULL");

		/*
		 * Missing fields if(tw.getRetweetedStatus() != null){
		 * prep.setLong(RETWEETED_STATUS, tw.getRetweetedStatus().getId()); }
		 * else{ prep.setLong(RETWEETED_STATUS, -1); }
		 */
		prep.setLong(RETWEETED_STATUS, -1);

		// prep.setArray(USER_MENTIONS, s.getUserMentionEntities());
		// prep.setArray(USER_MENTIONS, null);
		prep.setObject(USER_MENTIONS,
				userMentionsConvertor(tw.getUserMentionEntities()));

		prep.setObject(URLS, urlConvertor(tw.getURLEntities()));
		// prep.setArray(URLS, null);

		prep.setString(HASHTAGS, hashTagConvertor(tw.getHashtagEntities()));
		// prep.setObject(HASHTAGS, null);
		// prep.setArray(HASHTAGS, null);

		prep.setLong(USER, tw.getFromUserId());

	}

	/**
	 * Takes an instance of a User and prepares it for insert into the DB.
	 * Does so by using the prepared statement in conjunction with the constants for users.
	 * 
	...
	/**
	 * Converts the timestamp values from a status into a java timestamp compatible with the database.
	 * 
	...
	/**
	 * Converts the timestamp values from a tweet into a java timestamp campatible with the DB.
	 * 
	 * @deprecated tweets are converted into statuses so use the status convertor
	 * @param tw the tweet with the timestamp to be extracted
	 * @return a timestamp object
	 */
	public static Timestamp dateConvertor(Tweet tw) {
		Timestamp d = new Timestamp(tw.getCreatedAt().getTime());
		return d;

	}

	/**
	 * Converts the timestamp values from a user into a java timestamp compatible with the DB.
	 * 
	...
	/**
	 * method to insert multiple users form a list.
	 * @param conn Connection to the DB
